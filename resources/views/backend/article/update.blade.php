...
<div class="container">
    <form action="{{ route('article.update',$post->id) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <div class="card">
                    ...
                    <div class="form-group row @error('title') is-invalid @enderror">
                        ...
                        <div class="col-md-8">
                            <textarea name="description" id="description" class="form-control cke_wrapper
                                @error('description') is-invalid @enderror">{{ old('description',$post->content) }}</textarea>
                    ...
                    <div class="form-group row @error('status') is-invalid @enderror">
                        ...
                        <div class="col-md-3">
                            <select name="status" id="status"
                                ...
                                {{ $post->status == 'active' ? 'selected' : '' }}>Aktif</option>
                                <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}
                                {{ $post->status == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
